...
static NTSTATUS FileOpen(Drv_VirginityContext * pContext, UNICODE_STRING * pNtosName)
{
    NTSTATUS status = 0;
    HANDLE hFile = 0;
    ...
    // map file
    status = drv_MapAllFile(hFile, &pContext->m_mapped);
    if (!NT_SUCCESS(status))
        goto clean;
...
NTSTATUS Drv_VirginityInit(Drv_VirginityContext * pContext)
{
    ...
    // open file
    status = FileOpen(pContext, &ntosName);
    if (!NT_SUCCESS(status))
        goto clean;
